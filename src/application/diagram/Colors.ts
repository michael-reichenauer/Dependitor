...
export default class Colors {
  ...
  static createNodeBorderColors() {
    const colors: any = {};
    Colors.nodeColorNames().forEach(
      (name) => (colors[name] = Colors.getNodeColor(name).darker(darkerBorder))
    );
    return colors;
  }
  ...
  static createNodeFontColors() {
    const colors: any = {};
    Colors.nodeColorNames().forEach(
      (name) => (colors[name] = Colors.getNodeColor(name).getIdealTextColor())
    );
    return colors;
  }
  ...
  static createNodeHexColors() {
    const colors: any = {};
    Colors.nodeColorNames().forEach(
      (name) => (colors[name] = "#" + Colors.getNodeColor(name).hex())
    );
    return colors;
  }
  ...
  static createNodeBorderHexColors() {
    const colors: any = {};
    Colors.nodeColorNames().forEach(
      (name) => (colors[name] = "#" + Colors.getNodeBorderColor(name).hex())
    );
    return colors;
  }
  ...
  static createNodeFontHexColors() {
    const colors: any = {};
    Colors.nodeColorNames().forEach(
      (name) => (colors[name] = "#" + Colors.getNodeFontColor(name).hex())
    );
    return colors;
  }
}
